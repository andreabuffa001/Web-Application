...
function reg_register($data){
	...
	//$result=pg_query("SELECT currval(pg_get_serial_sequence('username','id'))");
...
function reg_clean_expired(){
	...
	WHERE (regdate + ".($_CONFIG['regexpire'] * 60 * 60).") <= ".time()." and temp='0'");
}
...
function reg_confirm($id){
	...
	return (pg_affected_rows () != 0) ? REG_SUCCESS : REG_FAILED;
}
?>